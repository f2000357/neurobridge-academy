...
// Importing a practice/assessment report (IXL, Khan, MAP) the guide exported and
  ...
  if (op === "import") {
    ...
    if (!doc) {
      return NextResponse.json(
        { error: "Upload the exported IXL / Khan report first (as an external report)." },
        { status: 400 }
      );
    }
    if (!aiEnabled) {
    ...
    const result = await planJsonFromDocs<Extracted>(
      "You read practice-tool progress reports (IXL, Khan Academy, MobyMax, MAP) and extract where a learner needs to work next. Be faithful to the report; do not invent skills. Plain text only.",
      `This is a progress/assessment report for ${child.name}. ` +
        ...
        `(IXL/Khan "ELA" splits into reading vs writing — reading/vocabulary/comprehension → reading; grammar/writing strategies → writing). ` +
        ...
        `JSON: {"provider": "ixl|khan|map", "summary": "one sentence", ` +
